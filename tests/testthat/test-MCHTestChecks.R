...
stat_gen_1 <- function(n) {
  x <- rnorm(n)
  sqrt(n) * mean(x)/sd(x)
}

stat_gen_2 <- function(n, mu) {
  x <- rnorm(n, mean = mu)
...
stat_gen_3 <- function(n, mu, sigma) {
  x <- rnorm(n, mean = mu, sd = sigma)
  sqrt(n) * (mean(x) - mu)/sigma
}

test_that("MCHTest() functions properly", {
  expect_is(MCHTest(test_stat_1, stat_gen_1, seed = 100), "MCHTest")
  expect_true(is.MCHTest(MCHTest(test_stat_1, stat_gen_1, seed = 100)))
  expect_warning(MCHTest(test_stat_1, stat_gen_1))
  expect_error(MCHTest(test_stat_1, stat_gen_1, seed = 100, test_params = "mu"))
  expect_error(MCHTest(test_stat_1, stat_gen_2, seed = 100, test_params = "mu"))
  expect_error(MCHTest(test_stat_2, stat_gen_2, test_params = c("mu", "sigma"),
               seed = 100))
  expect_silent(MCHTest(test_stat_2, stat_gen_2, test_params = "mu", seed = 10))
  expect_silent(MCHTest(test_stat_1, stat_gen_1, memoise_sample = FALSE))
  expect_error(MCHTest(test_stat_2, stat_gen_2, seed = 100, test_params = "mu",
                       fixed_params = "sigma"))
  expect_silent(MCHTest(test_stat_3, stat_gen_3, seed = 100, test_params = "mu",
                        fixed_params = "sigma"))
})

test_that("An MCHTest-class object functions properly", {
  mc.test.1 <- MCHTest(test_stat_1, stat_gen_1, seed = 100, N = 1000)
  mc.test.2 <- MCHTest(test_stat_1, stat_gen_1, N = 1000,
                       memoise_sample = FALSE)
  mc.test.3 <- MCHTest(test_stat_2, stat_gen_2, N = 100, seed = 100,
                       test_params = "mu", lock_alternative = FALSE)
  mc.test.4 <- MCHTest(test_stat_3, stat_gen_3, N = 100, seed = 100,
  ...
  expect_equal(mc.test.3(dat, alternative = "greater", mu = 0.5)$alternative,
               "greater")
  expect_error(MCHTest(test_stat_1, stat_gen_1, seed = 100, N = 5,
...
test_that("get_MCHTest_settings() functions properly", {
  f_setting <- get_MCHTest_settings(MCHTest(test_stat_2, stat_gen_2, seed = 100,
                                            memoise_sample = TRUE, N = 10000,
                                            test_params = "mu",
                                            lock_alternative = FALSE))
